...
class pb_backupbuddy_core {
	...
	function backup_integrity_check( $file, $fileoptions = '', $options = array() ) {
		...
		if ( ( 'full' == $backup_type ) || ( 'files' == $backup_type ) ) {
			$pass = false;
	...
	*/
	 ...
	 *
	 */
	public function alert_core_table_excludes( $excludes ) {
		global $wpdb;
		$prefix = $wpdb->prefix;
	...
	public function alert_core_file_excludes( $excludes ) {
		...
		$warn_dirs = array( // No trailing slash.
			...
			'/' . ltrim( str_replace( ABSPATH, '', pb_backupbuddy::$options['backup_directory'] ), '\\/' ),
		);
