...
class pb_backupbuddy_backup {
	...
	function pre_backup( $serial, $profile, $trigger, $pre_backup = array(), $post_backup = array(), $schedule_title = '', $export_plugins = array() ) {
		...
		*/
		...
		$fileExcludes = pb_backupbuddy::$classes['core']->alert_core_file_excludes( $this->_backup['directory_exclusions'] );
		...
		if ( '1' != $profile['skip_database_dump'] ) { // Backup database if not skipping.
			...
			$tableExcludes = pb_backupbuddy::$classes['core']->alert_core_table_excludes( $this->_backup['additional_table_excludes'] );
			...
			pb_backupbuddy::status( 'message', __( 'Skipping database dump based settings / profile type.', 'it-l10n-backupbuddy' ) . ' Backup type: `' . $type . '`.' );
		}
